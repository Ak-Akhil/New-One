...
namespace todo.Models
{
    public class ExcelEmail
    {
        ...
        public async Task EmailExcel(string id, string student_no)
        {
            ...
            Sheet.Cells["A1"].Value = "Student Number";
            Sheet.Cells["B1"].Value = "First Name";
            Sheet.Cells["C1"].Value = "Last Name";
            Sheet.Cells["D1"].Value = "Email Address";
            Sheet.Cells["E1"].Value = "Home Address";
            Sheet.Cells["F1"].Value = "Contact Number";
            Sheet.Cells["G1"].Value = "Photo Path";
            ...
            foreach (var item in collection)
            {
                ...
                Sheet.Cells[string.Format("A{0}", row)].Value = item.Student_Number;
                Sheet.Cells[string.Format("B{0}", row)].Value = item.First_Name;
                Sheet.Cells[string.Format("C{0}", row)].Value = item.Last_Name;
                Sheet.Cells[string.Format("D{0}", row)].Value = item.Email;
                Sheet.Cells[string.Format("E{0}", row)].Value = item.Home_Address;
                Sheet.Cells[string.Format("F{0}", row)].Value = item.Mobile;
                Sheet.Cells[string.Format("G{0}", row)].Value = item.Photo_Path;
